...
void CText::Render()
{
    ...
    if (m_FontStyle == L"None" || m_FontStyle.empty())
    {
        ...
        FontMgr::GetInst()->DrawFont(m_strText.c_str(), L"Arial", vScreenPos.x, vScreenPos.y, m_fFontSize, m_Color, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, m_Align);
    }
    else
    {
        // 커스텀 폰트 렌더링에 정렬 옵션 전달
        FontMgr::GetInst()->DrawFont(m_strText.c_str(), m_FontStyle.c_str(), vScreenPos.x, vScreenPos.y, m_fFontSize, m_Color, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, m_Align);
    }
}
