...
{
    if (!this->GetOwner()->GetIsActive() || !_Other->GetOwner()->GetIsActive()) return false;   // Active가 꺼진 오브젝트
    if (this->GetOwner()->IsDead() || _Other->GetOwner()->IsDead()) return false;             // 곧 삭제 처리될 오브젝트
