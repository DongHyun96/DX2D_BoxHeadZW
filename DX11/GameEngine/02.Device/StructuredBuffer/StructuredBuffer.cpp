...
HRESULT StructuredBuffer::Create(UINT _ElementSize, UINT _ElementCount, SB_TYPE _Type, bool _bSysMove, void* _SysMem)
{
    m_SB        = nullptr;
    m_SB_Write  = nullptr;
    m_SB_Read   = nullptr;
    ...
    m_SysMemMove    = _bSysMove;
    m_SBType        = _Type;

    m_Desc.ByteWidth = m_ElementSize * m_ElementCount;

    if (m_SBType == SRV_ONLY)     m_Desc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
    else if (m_SBType == SRV_UAV) m_Desc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS;
    ...
    else hr = DEVICE->CreateBuffer(&m_Desc, nullptr, m_SB.GetAddressOf());
	...
	else hr = DEVICE->CreateBuffer(&WriteDesc, nullptr, m_SB_Write.GetAddressOf());
	...
	else hr = DEVICE->CreateBuffer(&ReadBuffer, nullptr, m_SB_Read.GetAddressOf());
...
void StructuredBuffer::SetData(void* _SysMem, UINT _DataSize)
{
	...
	if (_DataSize == 0) _DataSize = m_Desc.ByteWidth;
...
{
	if (m_RecentRegisterNum == -1) return;
