   ...
   public class CHUD
   {
      ...
      private function UpdateScore() : void
      {
         ...
         if(lastScore != _Score && --mScoreUpdateTick < 0)
         {
            ...
            if(bmdScore)
            {
               _loc1_ = bmdScore.width < mcScore.width;
            }
      ...
      public function AddMessage(param1:Point, param2:String, param3:String, param4:Number = 1) : void
      {
         ...
         _loc7_ = {
            ...
            "pos":param1,
            "dpos":new Point(0,-3),
            "life":CGame.SECONDS * 3
         };
         _loc7_.mBlendMode = mDrawBlendMode;
      ...
      public function Process() : void
      {
         ...
         if(CKeyboard.IsDown("SHIFT") && CKeyboard.IsDown("CONTROL") && Boolean(CKeyboard.IsReleased("PAGE_UP")))
         {
         ...
         _loc5_ = int(mMessages.length);
         _loc6_ = 0;
         while(_loc6_ < _loc5_)
         {
            _loc8_ = mMessages[_loc6_];
            _loc9_ = _loc8_.pos;
            _loc10_ = _loc6_ + 1;
            while(_loc10_ < _loc5_)
            {
               _loc11_ = mMessages[_loc10_];
               _loc12_ = _loc11_.pos;
         ...
         for each(_loc7_ in mMessages.slice())
         {
            ...
            _loc2_.x *= 0.99;
            _loc2_.y *= 0.99;
            _loc7_.alpha = Math.min(_loc7_.maxAlpha,_loc7_.life >= CGame.SECONDS ? _loc7_.alpha + 0.1 : _loc7_.life / CGame.SECONDS);
            if(--_loc7_.life < 0)
            {
      ...
      public function Draw(param1:Rectangle, param2:CBitmapData) : void
      {
         ...
         UpdateScore();
         _loc3_.tx = param2.width - bmdScore.width - 1;
         _loc3_.ty = 0;
         dColorTransform.alphaMultiplier = mScoreAlpha;
         param2.draw(bmdScore,_loc3_,dColorTransform);
         dPoint = new Point(_loc3_.tx + mcScore.width / 2,_loc3_.ty + mcScore.height / 2);
         if(mScoreAlpha > 0.75)
         {
            mScoreAlpha -= 0.02;
         }
         _loc3_.tx = param2.width - mcMultiplier.width - 1;
         _loc3_.ty += bmdScore.height - 4;
      ...
      public function Scroll(param1:Point) : void
      {
         ...
         var _loc3_:Number = NaN;
         var _loc4_:Object = null;
         _loc2_ = Math.round(mRenderPos.x - param1.x);
         _loc3_ = Math.round(mRenderPos.y - param1.y);
         mRenderPos.x -= _loc2_;
         mRenderPos.y -= _loc3_;
