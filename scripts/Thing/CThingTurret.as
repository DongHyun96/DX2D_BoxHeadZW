package Thing
{
   ...
   public class CThingTurret extends CThingWeapon
   {
      ...
      public function CThingTurret(param1:Object, param2:* = null, param3:* = null)
      {
         ...
         mAffectFlags |= eAffectType.FILTER_SOLID | eAffectType.FILTER_PEEP;
         if(param3)
         {
      ...
      public function SetupRange(param1:Number, param2:Number) : void
      {
         ...
         while(_loc7_ <= mMaxRange)
         {
            ...
            {
               _loc9_ = _loc8_ * _loc8_ + _loc7_ * _loc7_;
               if(!(_loc9_ > _loc5_ || _loc9_ < _loc6_))
               {
                  _loc10_ = mMap.GetCellAt(_loc3_.x + _loc8_,_loc3_.y + _loc7_);
                  if(!(_loc10_.mCollide & eWMCCollideType.SOLID))
                  {
      ...
      {
         var _loc1_:CThingPeep = null;
         var _loc2_:Number = NaN;
         var _loc3_:CThingAngle = null;
         var _loc4_:CThingAngle = null;
         var _loc5_:Number = NaN;
         ...
         if(mSearchQuadrants_Monitor)
         {
            ...
            _loc4_ = mAngle.Clone();
            _loc5_ = 10000;
            _loc6_ = CWMCNavigation.SEARCHES[mAngle.mDirectionName];
            for each(_loc7_ in _loc6_)
            {
               ...
               for(_loc9_ in _loc8_)
               {
                  ...
                  if(_loc10_)
                  {
                     ...
                     {
                        _loc12_ = _loc10_.GetThings(mCollideID,eAffectType.FILTER_PEEP,100);
                        if(_loc12_.length != 0)
                        {
                           _loc11_ = false;
                        }
                        for each(_loc1_ in _loc12_)
                        {
                           if(_loc1_.mExists)
                           {
                              _loc13_ = mPosition.Distance2D(_loc1_.mPosition);
                              if(!(_loc13_ > mMaxRange || _loc13_ < mMinRange))
                              {
                                 _loc3_.mAngleRAD = mPosition.GetAngle(_loc1_.mPosition);
                                 _loc14_ = mAngle.TurnAngle(_loc3_).GetTurnCount(mTurnSpeed) * 0.2 + _loc13_ * 1;
                                 if(_loc14_ < _loc5_)
                                 {
                                    _loc5_ = _loc14_;
      ...
      override public function Render() : void
      {
         ...
         if(mcDrawInfo)
         {
            if(CHUD.mDraw != false)
            {
               if(mOwner)
               {
                  _loc1_ = Number(mOwner.mPosition.Distance2D(mPosition));
                  mcDrawInfo.alpha = _loc1_ <= 1.5 ? 1 : (_loc1_ < 2.5 ? 1 - (_loc1_ - 1.5) : 0);
